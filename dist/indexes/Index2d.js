  ...
  if (this._collection) {
    ...
    var collection = this._collection.subset({}, {
      ...
      "$orderBy": this._keys
        ...
        dataIndex,
        dataCount = collectionData.length; // Clear the index data for the index
...
Index2d.prototype.violation = function (dataItem) {
  // Generate item hash
  var uniqueHash = this._itemHash(dataItem, this._keys); // Check if the item breaks the unique constraint
...
Index2d.prototype.near = function (pathStr, query, options, op) {
  var self = this,
      ...
      i; // Calculate the required precision to encapsulate the distance
