		...
		var finalValue = function () {
			return value.reduce((itemArr, item) => {
				if (!Array.isArray(item) && Object.keys(item).length > 1) {
					...
					return itemArr.concat(reduceArray(objectToArray(item).map((item) => {
...
var queryToPipeline = function queryToPipeline (query) {
	...
	if (!currentGate) {
		...
		var gateKey = _match.gates.find((key) => {
	...
	return Object.entries(query).map((_ref4) => {
		...
		if (!valTypeData.isFlat) {
			...
			if (currentGate !== "$and") {
				...
				return objectToArray(value).map((item) => {
